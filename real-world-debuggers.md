...
## プロセス制御型を支える ptrace とソフトウェアブレークポイント
...
目的は同じで、手段が違う。この対応が見えると、`gdb` がぐっと身近になります。

なお、`ptrace` は他のプロセスのメモリを自由に読み書きできる強力な機能なので、
誰がどのプロセスに使えるかは OS の権限で制限されています。基本的には自分の
プロセスにしか使えず、Linux では Yama の `ptrace_scope` 設定により
「自分の子プロセス以外には attach できない」よう、さらに絞られていることもあります。
`gdb -p` でのアタッチが Operation not permitted で断られたら、この制限を疑ってください。
...
- **型情報とスコープ情報**: その変数がどんな型で、どの範囲で有効か。

つまり DWARF は、私たちが処理系の中に最初から持っていた情報（行番号や変数表）を、
コンパイル済みのプログラムのために外付けの表として用意したものだと言えます。
...
組み上げています[](#cite:sasada2021)。性能面の工夫にも注目してください。私たちの Toy は
毎文 `on_line` を素通しで呼びましたが、Ruby はイベントが購読されているときだけ
発火のコードが働くように実装されており、デバッガを使わない通常実行では
このフックのコストをほとんど払いません。「使わないときはタダ」にする設計は、
実用処理系がデバッグ機能を標準搭載するための鍵です。
...
おかげで、ひとつのエディタ（VS Code など）から、Ruby や Python、C++ など
