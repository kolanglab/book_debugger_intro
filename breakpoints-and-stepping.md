...
思い出してください。`exec_stmt` は、文を 1 つ実行する直前に必ず `on_line(line, stmt)` を
...
それがモードの切り替えになります。これがデバッガの基本的な動き方です。
...
ここで第 5 章の `call_func` を思い出してください。関数を呼ぶとフレームが 1 つ積まれ、
関数から戻るとフレームが 1 つ降ろされました。つまりコールスタックの深さ
...
> ステップインとオーバーとアウトの 3 種類すべてが表現できると分かります。
> ただし「深さ 1 つで足りる」のは、Toy の制御の流れが関数の呼び出しと戻りだけ、
> という単純さのおかげでもあります。実用言語には例外による大域脱出、`ensure` のような
> 後始末、ブロックや `yield`、async/await のような中断と再開があり、「呼んだら
> 同じ深さに戻ってくる」という前提自体が崩れます。実デバッガのステップ実行が
> フレームの同一性や例外経路まで見ているのは、このためです。
