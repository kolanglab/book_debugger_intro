...
`on_line` でその行に来たら条件をその場で評価して、真なら止めるだけ。
...
> [!WARNING]
> 「条件を評価器でその場で評価する」手軽さには、裏の顔もあります。条件式が**副作用**を
> 持っていたら（変数への代入や関数呼び出しを含んでいたら）、デバッガを付けたこと自体が
> プログラムの挙動を変えてしまいます。また、条件式の中で関数を呼び、その関数の中の行が
> さらにブレークポイントに当たったらどうなるでしょう。デバッガがデバッガを呼ぶ**再入**の問題です。
> 本書の Toy では `i == 3` のような純粋な式だけを想定しますが、実用デバッガは、
> 条件式に許す構文を制限したり、条件評価中はブレークポイントを無効にしたりして、
> この問題に備えています。
...
Lewis は、実行中の状態変化をすべて記録しておけば、あとから好きな時点の状態を
...
> 「再現」の鍵は**非決定的なもの**の記録です。計算そのものは同じ入力から同じ結果が
> 出ますが、時刻・乱数・ファイルやネットワークからの入力・スレッドの実行順序は
> 実行のたびに変わりえます。そこで record&replay では、これら「外の世界から来るもの」
> だけを記録しておき、再実行時に同じ値・同じ順序で与え直すことで、過去の実行を
> 正確になぞります。`rr` はこの方式で、現実のプログラムを低コストで記録・再生し、
...
本物のデバッガを並べて、どこが同じでどこが違うのかを確かめます。
